<div class="row mb-4">
    ...
    <div class="col-sm-10">
        <input name="title" type="text" id="title" placeholder="أدخل العنوان"
            value="{{ old('title', $about_us->title) }}" class="form-control @error('title') is-invalid @enderror">
        @error('title')
...
<div class="row mb-4">
    ...
    <div class="col-sm-10">
        <select name="status" class="form-select @error('status') is-invalid @enderror"">
            <option value="عرض" @if ($about_us->status == 'عرض' or old('status') == 'عرض') selected @endif>عرض
            </option>
            <option value="أخفاء" @if ($about_us->status == 'أخفاء' or old('status') == 'أخفاء') selected @endif>أخفاء
            </option>
        </select>
        @error('status')
            <p class="invalid-feedback">{{ $message }}</p>
